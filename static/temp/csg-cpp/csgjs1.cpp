...
int main()
{
    ...
    ifstream is("D:\\CodeProjects\\C++L\\test\\data.json", ios::binary);  //打开文件，以二进制方式打开文件
    if (!is.is_open())               //检查文件打开情况
        cout << "open file failed" << endl;

    if (reader.parse(is, root)) {
        //读取根节点信息
        Json::Value positions = root["positions"];
        csgjs_model a;

        vector<csgjs_vertex> vertices;
        for(unsigned int i=0;i<positions.size();i+=3){
            csgjs_vertex vertex;
            int v1=positions[i].asInt();
            int v2=positions[i+1].asInt();
            int v3=positions[i+2].asInt();
            vertex.pos=csgjs_vector(v1,v2,v3);
            vertex.normal=csgjs_vector();
            vertex.pos=csgjs_vector();
            vertices.push_back(vertex);
        }
        Json::Value indices = root["indices"];
        vector<int> newIndices;
        for(unsigned int i=0;i<indices.size();i++){
            newIndices.push_back(indices[i].asInt());
        }

        csgjs_model result=csgjs_difference(a,a);
        cout<<result.indices.size()<<endl;
        cout<<result.vertices.size()<<endl;
    }
    is.close();
    return 0;
}
